...
export const getShoppingCart = createAsyncThunk(
  ...
  async (jwt: string, thunkAPI) => {
    const { data } = await axios.get(
      `http://123.56.149.126:8089/api/shoppingCart`,
      {
        headers: {
...
export const addShoppingCartItem = createAsyncThunk(
  ...
  async (parameters: { jwt: string; touristRouteId: string }, thunkAPI) => {
    const { data } = await axios.post(
      `http://123.56.149.126:8089/api/shoppingCart/items`,
      {
...
export const checkout = createAsyncThunk(
  ...
  async (jwt: string, thunkAPI) => {
    const { data } = await axios.post(
      `http://123.56.149.126:8089/api/shoppingCart/checkout`,
      null,
      {
        headers: {
          Authorization: `bearer ${jwt}`,
...
export const clearShoppingCartItem = createAsyncThunk(
  ...
  async (parameters: { jwt: string; itemIds: number[] }, thunkAPI) => {
    return await axios.delete(
      `http://123.56.149.126:8089/api/shoppingCart/items/(${parameters.itemIds.join(
        ","
      )})`,
      {
        headers: {
