...
export const fetchHelpNumDataActionCreator = createAsyncThunk(
  ...
  async (goodsReviewHelpNum:{reviewId,ids}, thunkAPI) => {
    debugger;
    const { data } = await axios.post(
      `http://localhost:8081/helpNum`,goodsReviewHelpNum.reviewId
    );
    if(data!=null){
      debugger;
     // let page=1;
     let goodsId = 10700;
    thunkAPI.dispatch(fetchOpenAndCloseDataActionCreator({goodsId,(goodsReviewHelpNum.ids)))
    }
    return data;
  }
);
