...
export const LeftImgF1: React.FC<pIf> = ({ data }) => {
  ...
  const currentLeftImage=data.filter((num,index)=> index===count)
  const dispatch = useDispatch();
  return data === undefined ? (
    <h2>path</h2>
  ) : (
    <div className="container">
      {data.map((currentLeftImage, idx) => {
        ...
        return (
          <div className="mySlides" key={idx}>
            ...
            <img
              className="slgdemo"
              style={{ width: "100%" }}
              src={`http://localhost:8081${currentLeftImage.path}`}
              alt=""
            />
                          
          </div>
      ...
                  
      <div className="row">
        <a className="prev" onClick={() => dispatch(increment())}>
          ❮
        </a>
        {data.map((img, idx) => {
          return (
        ...
                 
        <a className="next" onClick={() => dispatch(decrement())}>
          ❯
        </a>
                                
      </div>
