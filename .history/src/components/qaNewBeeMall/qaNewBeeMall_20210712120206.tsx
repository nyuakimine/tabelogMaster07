...
import { decrement, increment} from "./qaPageSlice";
interface ifProps {
  goodsId: number;
  id: number;
  question: string;
...
export const QaNewBeeMall: React.FC<pIf> = ({data}) => {
  const qaPage = useSelector((state: RootState) => state.qaPageSlice.pagination);
  console.log("xxxxxxQadata", qaPage);
        ...
        <div className="zv-cqa">
          <div className="zv-cqa-step">
            <div className="zv-cqa-step-link">
              ...
              <span className="previousPage">   <button className="next" onClick={() => dispatch(decrement(qaPage))}>
              前ページ
        </button></span>
              <span>全2件</span>
              <span>
              ...
              <span className="nextPage"> <button className="prev" onClick={() => dispatch(increment(qaPage))}>
              次ページ
        </button></span>
            </div>
