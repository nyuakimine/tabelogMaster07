...
import {fetchQaInsertDataActionCreator} from "../../redux/qaInsertSlice/fetchQaInsertSlice"
...
export const QaNewBeeMall: React.FC<pIf> = ({ data }) => {
  ...
  let qaInsert = useSelector((state: RootState) => state.qaInsertSlice.data) 
  let totalPage = useSelector(
    (state: RootState) => state.qaPageSlice.data.totalPage
  );
  ...
  useEffect(() => {
    ...
    dispatch(fetchQaInsertDataActionCreator(goodsId));
   // dispatch(fetchQaInsertDataActionCreator(qaInsert));
  }, []);

  // const qaInsertFun = () => {
  //   let temp = input.current?.value
  //   input.
  //   dispatch(fetchQaInsertDataActionCreator(qaInsert));
    ...
    <section className="g-grid_item g-sm-block-sm" id="js-product-qa">
          ...
          <div className="zv-cqa-step">
            <div className="zv-cqa-step-link">
              <span
                ...
                onClick={() => {
                  if (qaPage > 1) {
                    return dispatch(fetchQaPagingDataActionCreator(qaPage - 1));
                  }
                }}
              >
                前ページ
              ...
              <span>全2件</span>
              <span>
                ページ<span id="currentPageNo">{qaPage}</span>/{totalPage}
              </span>
              <span
                className="nextPage"
                onClick={() => {
            ...
            return (
              <div className="zv-cqa delete" key={idx}>
                ...
                <div className="zv-cqa-q-info">
                  <span className="zv-cqa-q-reviewer">投稿日&nbsp; </span>
      ...
      <div id="ZVCQAPost">
        ...
        >
          ...
          <button type="button" id="ZVPostQuestionButton" className="zv-btn" onClick={() =>
            {let temp= input.current?.value
              input.current!.value = ''
              console.log('temptemptemp',temp)
              dispatch(fetchQaInsertDataActionCreator(temp,qaInsert))
            }
           }>
            質問を投稿
          </button>
