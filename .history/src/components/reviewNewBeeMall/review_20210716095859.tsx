...
export const ReviewNewBeeMall: React.FC<pIf> = () => {
  ...
  // let allIds = initialList.map((item) => item.id) && showMoreList.map((item) => item.id);
  //let del =
  ...
  const [buttonText, setButtonText] = useState(" レビューをもっと見る");
  ...
  const closeShowMoreReviewsBtn = (e) => {
     debugger;
    if (showMoreList) {
      // let closeClose = document.getElementsByClassName("closeShowMore")[0] as HTMLElement;
      // closeClose.style.display = "none";
      dispatch(closeShowList(showMoreList));
    }
    //changeText("閉じる")
  };
  // if (showMoreList === null) {
  //   debugger;
  //   // let closeClose = document.getElementsByClassName("closeCloseBtn")[0];
  //   // closeClose[0].style.display = "none";
  //   document.getElementById('closeLabel')!.style.display='none'
  // }
      ...
      >
        ...
        >
          ...
          <p className="g-align-tc" id="closeBtn">
            ...
            >
              <span
                className={showMoreList === null ? "closeCloseBtn" : undefined}
                id="closeLabel"
