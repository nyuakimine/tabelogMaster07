...
export const ReviewNewBeeMall: React.FC<pIf> = () => {
  let openAndClose = useSelector((state) => state.openCloseReviewSlice.data);
  let reviewUserInfo = useSelector(
    (state: RootState) => state.openCloseReviewSlice.reviewUserInfo
  );
  console.log('openAndCloseppppppppp',openAndClose);
  // let reviewIds = openAndClose.map(
  //   (reviewIds) => reviewUserInfo.ids === reviewIds.id
  // );
//    funcTwo:() => {
   let reviewIds = openAndClose.join((reviewIds) => reviewUserInfo.ids == reviewIds.id
  
    );
    console.log('reviewIdsppppppppp',reviewIds);
//  }
//  funcThree:() => {
//   console.log('箭头函数调用法')
// }
...
// }
  ...
  useEffect(() => {
    ...
    dispatch(fetchOpenAndCloseDataActionCreator(reviewUserInfo));
  }, []);
  return openAndClose === null ? (
    <h2>loading...</h2>
  ) : (
    <div>
      <div id="ZVCQAPost">
        ...
        <div id="js-replace">
          ...
          <div id="normal-productreview">
            ...
            <ul className="g-reviewList">
              {openAndClose.map((reviewList, idx) => {
                return (
                  <div key={idx}>
          ...
          <p className="g-align-tc" id="showMoreReviewsBtn">
            ...
            >
              ...
              <span
                id="showMoreLabel"
                onClick={() =>
                  dispatch(fetchOpenAndCloseDataActionCreator(openAndClose))
                }
              >
                レビューをもっと見る（3/
