...
export const ReviewNewBeeMall: React.FC<pIf> = () => {
  ...
  let reviewUserInfo = useSelector((state: RootState) => state.openCloseReviewSlice.reviewUserInfo);
  
  let reviewIds = openAndClose.map((reviewIds) =>
  reviewUserInfo.ids === reviewIds.id
    );

  //let reviewIds = reviewUserInfo.ids
  //let addIds =data.join

  // const currentBigImage = data.filter(
  //   (num, index) => index === count
  // )[0] as ifProps;
 // console.log("xxxxxxReviewUserInfo", reviewUserInfo);
      ...
      >
        ...
        >
          <p className="g-align-tc" id="showMoreReviewsBtn">
            ...
            onClick={() => dispatch(fetchOpenAndCloseDataActionCreator(reviewIds))}
              >
